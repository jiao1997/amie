 ...
 * distinguish between centuries and millenias).
 */
...
public class DateParser {
	...
	public static final String newDate(int y, int m, int d) {
		return (newDate("" + y, m < 10 ? "0" + m : "" + m, d < 10 ? "0" + d
				: "" + d));
	}
	...
	public static final String DATE = newDate("(-?[0-9#X]++)",
			"([0-9#X]{1,2})", "([0-9#X]{1,2})");

	public static final Pattern DATEPATTERN = Pattern.compile(WBNEG + DATE); // No
																				// trailing
																				// boundary
																				// because
																				// of
	...
	public static final Pattern JUSTDATEPATTERN = Pattern.compile(DATE); // No
																			// trailing
																			// boundary
																			// because
																			// of
	...
	public static final String SDATE = newSubDate("(-?[0-9#X]++)",
			"([0-9#X]{1,2})");

	public static final Pattern SDATEPATTERN = Pattern.compile(WBNEG + SDATE); // No
																				// trailing
																				// boundary
																				// because
																				// of
	...
	private static final String[] MONTHS_EN = new String[] { "Jan", "Feb",
			"Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov",
			"Dec" };

	private static final String[] MONTHS_DE = new String[] { "Jan", "Feb",
			"M\u00E4r", "Apr", "Mai", "Jun", "Jul", "Aug", "Sep", "Okt", "Nov",
			"Dez" };

	private static final String[] MONTHS_ES = new String[] { "ene", "feb",
			"mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov",
			"dic" };

	private static final String[] MONTHS_FR = new String[] { "janv",
			"f\u00E9vr", "mars", "avr", "mai", "juin", "juil", "ao\u00FBt",
			"sept", "oct", "nov", "d\u00E9c" };

	private static final String[] MONTHS_IT = new String[] { "gen", "feb",
			"mar", "apr", "mag", "giu", "lug", "ago", "sep", "ott", "nov",
			"dic" };
	...
	// TODO: could also be internationalized
	...
	private static final FindReplace[] patterns = new FindReplace[] {
			...
			// --------- Process ISO8601 ------------------
			new FindReplace(Y + " ?[-\\|] ?(\\d{1,2}) ?[-\\|] ?(\\d{1,2})",
					newDate("$1", "$2", "$3")),

			// --------- Process ISO8601 include Month------------------
			new FindReplace(Y + " ?[-\\|] ?" + M + " ?[-\\|] ?(\\d{1,2})",
					newDate("$1", "$2", "$3")),

			// --------- Process BC, CE and AD ------------
			// 2267 - 2213 BC
			new FindReplace(Y + H + Y + BC, newDate("-$1", "##", "##") + " to "
					+ newDate("-$2", "##", "##")),
			// 2267 - 2213 CE
			new FindReplace(Y + H + Y + CE, newDate("$1", "##", "##") + " to "
					+ newDate("$2", "##", "##")),
			// 1000 BC - AD 120
			new FindReplace(Y + BC + H + AD + Y, newDate("-$1", "##", "##")
					+ " to " + newDate("$2", "##", "##")),
			// 1000 BC - 120 CE
			new FindReplace(Y + BC + H + Y + CE, newDate("-$1", "##", "##")
					+ " to " + newDate("$2", "##", "##")),
			// AD 46 - 120
			new FindReplace(AD + Y + H + Y, newDate("$1", "##", "##") + " to "
					+ newDate("$2", "##", "##")),
			// 1000 BC
			...
			new FindReplace(WB + N + H + N + B + "(?:of)?" + B + M + B + Y,
					newDate("$4", "$3", "$1") + " to "
							+ newDate("$4", "$3", "$2")),
			// November 23rd - 24th 1998
			new FindReplace(M + B + N + H + N + FB + Y, newDate("$4", "$1",
					"$2") + " to " + newDate("$4", "$1", "$3")),
			// November 23rd - March 24th 1998
			new FindReplace(M + B + N + H + M + B + N + FB + Y, newDate("$5",
					"$1", "$2") + " to " + newDate("$5", "$3", "$4")),
			// November 23[rd] 1998
			new FindReplace(M + B + N + FB + Y, newDate("$3", "$1", "$2")),
			// November 23, 1998
			new FindReplace(M + B + N + B + "," + B + Y, newDate("$3", "$1",
					"$2")),
			// 23[rd] November 1998
			new FindReplace(N + B + M + B + Y, newDate("$3", "$2", "$1")),
			// the 23[rd] of November 1998
			new FindReplace("the " + N + B + "of" + B + M + B + Y, newDate(
					"$3", "$2", "$1")),
			// 23[rd] of November 1998
			new FindReplace(N + B + "of" + B + M + B + Y, newDate("$3", "$2",
					"$1")),
			// 1.11.1998
			new FindReplace(N + "\\." + N + "\\." + Y,
					newDate("$3", "$2", "$1")),
			// 12-Sep-1970
			...
			// June - April 1980
			new FindReplace(M + H + M + B + Y4, newDate("$3", "$1", "##")
					+ " to " + newDate("$3", "$2", "##")),
			// April 1980
			...
			// June 14th - July 17th
			new FindReplace(M + B + N + H + M + B + N,
					newDate("##", "$1", "$2") + " to "
							+ newDate("##", "$3", "$4")),
			// June 14th - 17th
			new FindReplace(M + B + N + H + N, newDate("##", "$1", "$2")
					+ " to " + newDate("##", "$1", "$3")),
			// June 14th
			...
			// -----------
			// ##th - ##th millennium BC
			new FindReplace(THE + NTH + H + NTH + B + MILENNIUM + BC, ' '
					+ newDate("-&DEC$1###", "##", "##") + " to "
					+ newDate("-&DEC$2###", "##", "##")),
			// ##th - ##th century BC
			new FindReplace(THE + NTH + H + NTH + B + CENTURY + BC, ' '
					+ newDate("-&DEC$1##", "##", "##") + " to "
					+ newDate("-&DEC$2##", "##", "##")),
			// ##th - ##th century
			new FindReplace(THE + NTH + H + NTH + B + CENTURY, ' '
					+ newDate("&DEC$1##", "##", "##") + " to "
					+ newDate("&DEC$2##", "##", "##")),
			// ##th millennium BC
			new FindReplace(THE + NTH + B + MILENNIUM + BC, ' ' + newDate(
					"-&DEC$1###", "##", "##")),
			// ##th century BC
			new FindReplace(THE + NTH + B + CENTURY + BC, ' ' + newDate(
					"-&DEC$1##", "##", "##")),
			// ##th millennium
			new FindReplace(THE + NTH + B + MILENNIUM, ' ' + newDate(
					"&DEC$1###", "##", "##")),
			// ##th century
			...
			new FindReplace(newDate("([0-9#]++)", "([0-9])", "([0-9\\#]++)"),
					newDate("$1", "0$2", "$3")),
			new FindReplace(
					newDate("([0-9#]++)", "([0-9#]{2})", "([0-9])" + WB),
					newDate("$1", "$2", "0$3")) };

	/** Holds the pattern seeking for month names */
	// private static final Pattern monthPattern = Pattern.compile(WB +
	...
	private static final Pattern monthPatternEn = Pattern
			.compile(WB
					+ "(Jan|January|Feb|February|Febr|Mar|March|Apr|April|May|Jun|June|Jul|July|Aug|August|Sep|September|Sept"
					+ "|Oct|October|Nov|November|Dec|December)" + WB);

	private static final Pattern monthPatternDe = Pattern
			.compile(WB
					+ "(Jan|Januar|Feb|Februar|Febr|M\u00E4r|M\u00E4rz|Apr|April|Mai|Jun|Juni|Jul|Juli|Aug|August|Sep|September|Sept"
					+ "|Okt|Oktober|Nov|November|Dez|Dezember)" + WB);

	private static final Pattern monthPatternEs = Pattern
			.compile(WB
					+ "(ene|enero|feb|febrero|febr|mar|marzo|abr|abril|may|mayo|jun|junio|julio|ago|agosto|sep|septiembre"
					+ "|oct|octubre|nov|noviembre|dic|diciembre)" + WB);

	private static final Pattern monthPatternFr = Pattern
			.compile(WB
					+ "(janv|janvier|f\u00E9vr|f\u00E9vrier|mars|avr|avril|mai|juin|juil|juillet|ao\u00FBt|sept|septembre"
					+ "|oct|octobre|nov|novembre|d\u00E9c|d\u00E9cembre)" + WB);

	private static final Pattern monthPatternIt = Pattern
			.compile(WB
					+ "(gen|gennaio|feb|febbraio|febr|mar|marzo|apr|aprile|mag|maggio|giu|giugno|lug|luglio|ago|agosto|set|settembre"
	...
	/**
	 * Normalizes all dates in a String DEPRECATED - can go away when
	 * multilingual version accepted
	 */
	/*
	 * public static String normalize(CharSequence s) {
	 * 
	 * // If it does not contain years or months, return it unchanged if
	 * (!monthPattern.matcher(s).find() && !yearPattern.matcher(s).find())
	 * return (s.toString());
	 * 
	 * StringBuffer in = new StringBuffer((int) (s.length() * 1.1));
	 * 
	 * // Replace all the months Matcher m = monthPattern.matcher(s); while
	 * (m.find()) { int monthNum = D.indexOf(m.group().substring(0, 3),
	 * (Object[]) MONTHS) + 1; m .appendReplacement(in, "MONTH" + (monthNum /
	 * 10) + (monthNum % 10)); } m.appendTail(in); StringBuffer out = new
	 * StringBuffer((int) (in.length() * 1.1));
	 * 
	 * // Apply the patterns for (FindReplace p : patterns) { m =
	 * p.pattern.matcher(in); if (m.find()) { out.setLength(0); do {
	 * m.appendReplacement(out, p.replacement); } while (m.find());
	 * m.appendTail(out); StringBuffer temp = out; out = in; in = temp; } }
	 * 
	 * m = Pattern.compile("&DEC(\\d++)").matcher(in); if (m.find()) {
	 * out.setLength(0); do { m.appendReplacement(out, "" +
	 * (Integer.parseInt(m.group(1)) - 1)); } while (m.find());
	 * m.appendTail(out); in = null; return (out.toString()); } out = null;
	 * return (in.toString()); }
	 */
	...
	public static String normalize(CharSequence s, Language language) {
		...
		while (m.find()) {
			int monthNum = D.indexOf(m.group().substring(0, 3),
					(Object[]) months);
			if (monthNum == -1)
				monthNum = D.indexOf(m.group().substring(0, 4),
						(Object[]) months);
	...
	public static String normalize(CharSequence s, Language language,
		...
		for (FindReplace p : patterns) {
			...
			if (m.find()) {
				out.setLength(0);
				do {
					m.appendReplacement(out, p.replacement); // TODO: solve this
																// more
																// efficiently?
	...
	public static Collection<Triple<String, Integer, Integer>> getAllDatePositions(
			CharSequence s) {
	...
	/** Returns all the dates values and their position in the text */
	public static Collection<Triple<String, Integer, Integer>> getAllDatePositions(
			CharSequence s, Language language) {
		Pattern monthPattern;
		String[] months = null;
		if (language.equals(Language.ENGLISH)) {
			months = MONTHS_EN;
		...
		while (m.find()) {
			...
			Triple<Integer, Integer, Integer> offsetPair = new Triple<Integer, Integer, Integer>(
					position, m.end() - m.start(), diff);
			...
			if (m.find()) {
				...
				do {
					...
					if (tt.find()) {
						...
						Iterator<Triple<Integer, Integer, Integer>> iter = offSetList
								.iterator();
						previousOffSet = 0;
						boolean ignore = false;
						while (iter.hasNext()) {
							Triple<Integer, Integer, Integer> nextOffSetPair = iter
									.next();
							if (start <= nextOffSetPair.first
									&& end >= nextOffSetPair.first) {
								offSetPairs.add(nextOffSetPair);
							} else if (nextOffSetPair.first < start) {
								previousOffSet = previousOffSet
										+ nextOffSetPair.third;
							}
							if (nextOffSetPair.first < start
									&& nextOffSetPair.second
											+ nextOffSetPair.first >= end) {
								ignore = true;
							}
						}
						if (ignore) {
						...
						if (offSetPairs.size() > 0) {
							...
							while (iter.hasNext()) {
								Triple<Integer, Integer, Integer> offSetPair = iter
										.next();
							...
							Triple<Integer, Integer, Integer> newOffSetPair = new Triple<Integer, Integer, Integer>(
									position, o1.length(), length);
						...
						if (!offsetUsed.contains(start)) {
							dates.add(new Triple<String, Integer, Integer>(o1
									.toString(), start, end));
				...
				out = in;
				in = temp;
				Iterator<Triple<Integer, Integer, Integer>> iter = toRemove
						.iterator();
				while (iter.hasNext()) {
			...
			do {
				...
				if (tt.find()) {
					...
					while (iter.hasNext()) {
						...
						if (nextOffSetPair.first < start
								&& nextOffSetPair.second + nextOffSetPair.first >= end) {
							ignore = true;
						}
					}
					if (ignore) {
	 ...
	 * 
	 * 
	 * ArrayList<String> dates = new ArrayList<String>(); // If it does not
	 * contain years or months, return it unchanged if
	 * (!monthPattern.matcher(s).find() && !yearPattern.matcher(s).find())
	 * return (null);
	 * 
	 * StringBuffer in = new StringBuffer((int) (s.length() * 1.1));
	 ...
	 * 
	 * // Apply the patterns for (FindReplace p : patterns) { m =
	 * p.pattern.matcher(in); if (m.find()) { out.setLength(0); do { String
	 * retrivel = m.group(); StringBuffer tem = new StringBuffer(retrivel);
	 * m.appendReplacement(out, p.replacement); Matcher tt =
	 * p.pattern.matcher(tem); StringBuffer o1 = new StringBuffer(); if
	 * (tt.find()) { tt.appendReplacement(o1, p.replacement);
	 * dates.add(o1.toString()); }
	 * 
	 * } while (m.find()); m.appendTail(out); StringBuffer temp = out; out = in;
	 * in = temp; } }
	 * 
	 * m = Pattern.compile("&DEC(\\d++)").matcher(in); if (m.find()) {
	 * out.setLength(0); do { String retrivel = m.group(); StringBuffer tem =
	 * new StringBuffer(retrivel); m.appendReplacement(out, "" +
	 * (Integer.parseInt(m.group(1)) - 1)); Matcher tt =
	 * Pattern.compile("&DEC(\\d++)").matcher(tem); StringBuffer o1 = new
	 * StringBuffer(); if (tt.find()) { tt.appendReplacement(o1, "" +
	 * (Integer.parseInt(m.group(1)) - 1)); dates.add(o1.toString()); }
	 * 
	 * } while (m.find()); m.appendTail(out); in = null; //
	 * return(out.toString()); } out = null; return dates; }
	 */
	...
	public static List<String> getDates(CharSequence d) {
		...
		for (String date : preresult) {
			...
			// check year does not start with a symbol (due to regular
			// expression)
			if (!split[0].contains("#")) {
				try {
	...
	public static int[] asInts(String[] yearMonthDay) {
		...
		for (int i = 0; i < 3; i++) {
			...
			// ####-##-##
			{
				...
				result[i] = Integer.parseInt(yearMonthDay[i]);
		}
		return (result);
	}
	...
	public static boolean isEarlier(String[] date1, String[] date2) {
		return (isEarlier(asInts(date1), asInts(date2))); // TODO does not work
		// for wildcards in
		// years
	}
	...
	public static void main(String[] argv) throws Exception {
		System.out.println(NumberFormatter.ISOtime(DateParser
