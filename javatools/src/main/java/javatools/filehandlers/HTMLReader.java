...
public class HTMLReader extends Reader {
	...
	public int read() throws IOException {
		int c = in.read();
		if (announceChars != -1)
			Announce.progressAt(announceChars++);
		switch (c) {
		case -1:
			return (-1);
		case '<': {
			...
			if (announceChars != -1)
				Announce.progressAt(announceChars += t.length());
			...
			if (t.length() > 0)
				t.setLength(t.length() - 1);
			tagContent = Char17.decodePercentage(Char17.decodeAmpersand(t
					.toString()));
		...
		case '&': {
			String a = "&";
			while (!Character.isWhitespace((char) (c = in.read())) && c != ';'
					&& c != -1)
				a += (char) c;
			c = Char17.decodeAmpersandChar(a);
			if(c==-1) c=0;
			if (announceChars != -1)
				Announce.progressAt(announceChars += a.length());
			return (c);
		}
		default:
			return (c);
