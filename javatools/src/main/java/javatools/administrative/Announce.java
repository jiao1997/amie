...
public class Announce {
	...
	public static void printTime(String name, int timerNumber) {
		if (name != null)
			message("Time", name, ":",
					NumberFormatter.formatMS(getTime(timerNumber)));
		else
	...
	public static void errorException(Object... o) {
		...
		for (Object ob : o)
			if (ob instanceof Exception)
				cause = (Exception) ob;
		throw cause != null ? new RuntimeException("Fatal Error.", cause)
	...
	public static void progressShowTime() {
		...
		print("("
				+ NumberFormatter
						.formatMS((long) ((System.currentTimeMillis() - progressStart[progressLevel])
								* (progressEnd[progressLevel] - progressCounter[progressLevel]) / progressCounter[progressLevel]))
				+ " to go)");
